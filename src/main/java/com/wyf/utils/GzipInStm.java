...
public class GzipInStm extends InputStream {
  ...
  private ByteArrayOutputStream bout;//压缩后的内容会存放在此流中,然后会将此流的内容转给结果流
  ...
  public int read(byte b[], int off, int len) throws IOException {
    ...
    int rlen = 0;
    while (true) {//循环读取内容
      rlen += IOUtil.stmTryRead(this.rin, b, off + rlen, len - rlen);
  ...
  private void initStm() throws IOException {
    ...
    bout = new ByteArrayOutputStream(BUF_SIZE);
    gout = new GZIPOutputStream(bout);
    //此处不能调用readNextData,因为readNextData中会重置bout
    IOUtil.stmCopyFrom(this.oriin, gout, BUF_SIZE);
    this.rin = new ByteArrayInputStream(bout.toByteArray(),0,bout.size());
    init = true;
  }
  ...
  private void readNextData() throws IOException {
    ...
    long r = 0;
    while (bout.size() == 0) {
      r = IOUtil.stmCopyFrom(this.oriin, gout, BUF_SIZE);
      if (r == 0) {
        gout.finish();
        gout.close();
        isend = true;
